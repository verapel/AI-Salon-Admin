-- WA-FIX-1: Fix Postgres MIN(uuid) in commit_whatsapp_booking_owned
-- + salon|phone advisory serialization before client lookup/create.
-- Corrective REPLACE only. Does NOT change booking UX / identity model.
-- Does NOT touch Telegram / Instagram / Apple runtime.
-- Does NOT add clients.phone unique constraint.
--
-- Defect A: SELECT COUNT(*), MIN(c.id) fails — PostgreSQL has no MIN(uuid).
-- Fix A: (array_agg(c.id ORDER BY c.id::text))[1] preserves:
--   - salon-scoped phone digit lookup
--   - count for ambiguous_client (count > 1)
--   - single UUID when count = 1
--   - NULL when count = 0 (create-client path)
--
-- Defect B: staff|date advisory lock does not serialize same-salon same-phone
-- first-time commits across different slots. Concurrent count=0 paths can each
-- INSERT a clients row; the later identity insert then hits
-- (salon,provider,normalized_address) unique and returns identity_conflict
-- WITHOUT cleaning the provisional client → orphan duplicate client.
-- Fix B: pg_advisory_xact_lock(hashtext('whatsapp-client|'||salon), hashtext(digits))
-- acquired BEFORE phone client SELECT/INSERT. Different salons use different k1.
--
-- Based on live function after 20260805000003 (whatsapp_booking_recovery).
...
BEGIN
  ...
  -- WA-FIX-1: Serialize same-salon same-phone client resolution before SELECT/INSERT.
  -- Independent of staff|date booking lock (different slots must still not duplicate clients).
  ...
  -- WA-FIX-1: Postgres has no MIN(uuid). Preserve count + single-id capture.
