-- WA-4D1: Idempotent atomic WhatsApp booking commit.
-- Additive only. SERVICE ROLE / backend only.
-- Does NOT send messages, create Telegram reminders, or call Meta.
--
-- Guarantees:
--   1) Receipt ownership locked before business mutations
--   2) Conversation CAS: booking + ready_to_book + sourceMessageId
--   3) Appointment unique (salon_id, source, source_external_event_id)
--   4) Staff/date advisory lock + interval overlap recheck
--   5) Client + identity + appointment + conversation completion atomic
--   6) No orphan client on booking failure (validations before INSERT client;
--      remaining failures RAISE / unique re-read paths)
--   7) WA-4D2: existing clients with is_blocked refuse with client_blocked
--      (identity / conversation / phone) before identity or appointment writes
--   8) WA-4D2: conversation client ≠ phone client (no identity) →
--      client_resolution_conflict (no silent phone-identity attach)

BEGIN;

-- ---------------------------------------------------------------------------
-- A. appointments.source: allow 'whatsapp' (preserve telegram|owner|apple)
-- ---------------------------------------------------------------------------
ALTER TABLE public.appointments
  ADD COLUMN IF NOT EXISTS source_external_event_id TEXT NULL;

COMMENT ON COLUMN public.appointments.source_external_event_id IS
  'Idempotency key for channel-originated bookings (e.g. WhatsApp channel_event_receipts.external_event_id). Unique with (salon_id, source) when both source and this column are non-null.';
...
ALTER TABLE public.appointments
  ...
  CHECK (source IS NULL OR source IN ('telegram', 'owner', 'apple', 'whatsapp'));
...
-- ---------------------------------------------------------------------------
-- B. commit_whatsapp_booking_owned
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF FOUND THEN
    -- Repair conversation completion if needed (idempotent).
    ...
    ELSIF v_conv.client_id = v_existing.client_id THEN
      UPDATE public.channel_conversations c
      SET
        current_flow = NULL,
        current_step = NULL,
  ...
  EXCEPTION WHEN unique_violation THEN
    ...
    -- N) Complete conversation for already_booked race.
    IF v_conv.client_id IS NULL THEN
      UPDATE public.channel_conversations c
      SET
        ...
        updated_at = v_now
      WHERE c.id = v_conv.id;
    ELSIF v_conv.client_id = v_appt.client_id THEN
      UPDATE public.channel_conversations c
      SET
        current_flow = NULL,
        current_step = NULL,
