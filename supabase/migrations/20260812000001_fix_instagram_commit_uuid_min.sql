-- IG-ACTIVATE-2F: Fix Postgres MIN(uuid) in commit_instagram_booking_owned.
-- Corrective REPLACE only. Does NOT change booking semantics.
-- Does NOT touch Telegram / Apple / WhatsApp runtime.
--
-- Defect: SELECT COUNT(*), MIN(c.id) fails — PostgreSQL has no MIN(uuid).
-- Fix: (array_agg(c.id ORDER BY c.id::text))[1] preserves:
...
-- Phone advisory lock / identity / appointment insert logic unchanged.
