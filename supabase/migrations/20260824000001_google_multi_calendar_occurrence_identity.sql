-- GOOGLE MULTI-CALENDAR: occurrence identity includes external_calendar_id.
-- Manual import RPC then accepts every calendar in provider_config.selectedCalendars
-- (or the primary selected_calendar_id). FAST-6 auto-import stays primary-only.
-- Additive / backward-compatible. Does NOT drop rows.
--
...
-- ---------------------------------------------------------------------------
-- A. appointment_external_links: uniqueness includes calendar id
-- ---------------------------------------------------------------------------

-- Preserve already-populated calendar ids. Fill Google NULLs from the
-- connection primary so legacy single-calendar rows keep matching.
UPDATE public.appointment_external_links AS l
SET external_calendar_id = NULLIF(BTRIM(c.selected_calendar_id), '')
FROM public.calendar_connections AS c
WHERE l.calendar_connection_id = c.id
  AND l.provider = 'google'
  AND COALESCE(NULLIF(BTRIM(l.external_calendar_id), ''), '') = ''
  AND COALESCE(NULLIF(BTRIM(c.selected_calendar_id), ''), '') <> '';

ALTER TABLE public.appointment_external_links
  DROP CONSTRAINT IF EXISTS appointment_external_links_uid_recurrence_unique;

CREATE UNIQUE INDEX IF NOT EXISTS appointment_external_links_connection_calendar_uid_recurrence_unique
  ON public.appointment_external_links (
...
COMMENT ON INDEX public.appointment_external_links_connection_calendar_uid_recurrence_unique IS
  'Google multi-calendar occurrence identity: connection + calendar + uid + recurrence. COALESCE keeps NULL recurrence / calendar aligned with legacy empty-string semantics.';

-- Google occurrence identity is appointment_external_links
-- (connection + calendar + uid + recurrence). The appointments unique
-- (salon_id, source, source_external_event_id) would collapse the same
-- Google eventId from two selected calendars. Exclude source='google'
-- only. WhatsApp / Instagram / Apple keep this unique index.
DROP INDEX IF EXISTS public.appointments_salon_source_external_event_uidx;
...
CREATE UNIQUE INDEX IF NOT EXISTS appointments_salon_source_external_event_uidx
  ...
  WHERE source IS NOT NULL
    ...
    AND source IS DISTINCT FROM 'google';

COMMENT ON INDEX public.appointments_salon_source_external_event_uidx IS
  'Channel booking idempotency for non-Google sources. Google occurrences are unique on appointment_external_links (connection + calendar + uid + recurrence).';

-- ---------------------------------------------------------------------------
-- B. calendar_import_issues: calendar column + open uniqueness
-- ---------------------------------------------------------------------------

ALTER TABLE public.calendar_import_issues
  ADD COLUMN IF NOT EXISTS external_calendar_id TEXT NULL;

COMMENT ON COLUMN public.calendar_import_issues.external_calendar_id IS
  'External calendar id for the occurrence (Google calendarId). NULL on legacy Apple / unscoped rows.';

-- Prefer the id already stored on the overlay payload; otherwise the
-- Google connection primary. Do not invent ids for non-Google rows.
UPDATE public.calendar_import_issues AS i
SET external_calendar_id = COALESCE(
  NULLIF(BTRIM(i.raw_event->>'calendarId'), ''),
  NULLIF(BTRIM(c.selected_calendar_id), '')
)
FROM public.calendar_connections AS c
WHERE i.calendar_connection_id = c.id
  AND c.provider = 'google'
  AND COALESCE(NULLIF(BTRIM(i.external_calendar_id), ''), '') = ''
  AND COALESCE(
    NULLIF(BTRIM(i.raw_event->>'calendarId'), ''),
    NULLIF(BTRIM(c.selected_calendar_id), '')
  ) IS NOT NULL;

DROP INDEX IF EXISTS calendar_import_issues_open_event_unique;

CREATE UNIQUE INDEX IF NOT EXISTS calendar_import_issues_open_event_unique
  ON public.calendar_import_issues (
    calendar_connection_id,
    COALESCE(external_calendar_id, ''),
    external_uid,
    COALESCE(recurrence_id, '')
...
-- ---------------------------------------------------------------------------
-- C. RPC: accept any selected Google calendar for this salon connection
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  v_start_time time;
  v_end_time time;
  v_calendar_allowed boolean := false;
BEGIN
  ...
  -- Connection must belong to salon + google.
  SELECT *
  INTO v_conn
  ...
  -- Primary selected_calendar_id OR an id in provider_config.selectedCalendars.
  v_calendar_allowed :=
    coalesce(nullif(btrim(v_conn.selected_calendar_id), ''), '') = v_ext_cal
    OR (
      jsonb_typeof(COALESCE(v_conn.provider_config, '{}'::jsonb)->'selectedCalendars') = 'array'
      AND EXISTS (
        SELECT 1
        FROM jsonb_array_elements(
          COALESCE(v_conn.provider_config, '{}'::jsonb)->'selectedCalendars'
        ) AS elem
        WHERE coalesce(nullif(btrim(elem->>'id'), ''), '') = v_ext_cal
      )
    );

  IF NOT v_calendar_allowed THEN
    RETURN jsonb_build_object('kind', 'error', 'code', 'google_calendar_not_selected');
  END IF;

  -- Idempotent: existing external occurrence link (calendar-scoped, with
  -- legacy unscoped-primary fallback so pre-migration rows still match).
  SELECT *
  ...
  WHERE l.calendar_connection_id = p_calendar_connection_id
    ...
    AND (
      coalesce(l.external_calendar_id, '') = v_ext_cal
      OR (
        coalesce(l.external_calendar_id, '') = ''
        AND coalesce(nullif(btrim(v_conn.selected_calendar_id), ''), '') = v_ext_cal
      )
    )
  FOR UPDATE;
  ...
  -- Do not dedupe Google occurrences by appointments.source_external_event_id.
  -- Equal eventIds in different selected calendars are distinct; identity is
  -- appointment_external_links (connection + calendar + uid + recurrence).
  ...
  EXCEPTION
    WHEN unique_violation THEN
      IF v_client_created THEN
        DELETE FROM public.clients WHERE id = v_client_id AND salon_id = p_salon_id;
      END IF;
      -- Resolve through calendar-scoped external links, never bare eventId.
...
COMMENT ON FUNCTION public.commit_google_calendar_manual_import IS
  'GOOGLE multi-calendar: atomic manual one-event Google import. Accepts selected_calendar_id or provider_config.selectedCalendars. No Google writes. No reminders.';
